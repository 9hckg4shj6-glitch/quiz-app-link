...
|---|---|---|
| 1 | データ型と検証 | 完了 |
| 2 | 通常演習（テキスト・数値・作図・自己採点） | 完了 |
| 3 | 答案履歴（Dexie v2）・誤答ノート・バックアップv2 | 完了 |
| 4 | 選択＋記述の混在本番モード | 完了 |
| 5 | 差分同期（migration 008・RPC・クライアント） | 完了（Supabase側の migration 適用は未実施） |
| 6 | 免疫コンテンツ | 令和6年度（マーク80問＋記述6問）を収録。R3〜R5は未収録 |

実装上、仕様書と1点だけ異なる扱いをしている箇所がある。§9.3 は「本番モードでは
FSRS復習イベントを作成しない」としているが、既存の本番モード（選択問題）は
提出時に `scheduleSRS` を呼んでいる。「既存本番モードと同様」という文言の意図を
優先し、記述問題も自己採点の確定時に満点=3／部分点=2／0点=1 で `scheduleSRS` を
呼んでいる（個別の手ごたえボタンは出さない、という点は仕様どおり）。
