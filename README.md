...
有効化には `supabase/migrations/003_community.sql` を SQL Editor で実行します（`boards` / `posts` / `app_secrets` テーブルと RPC 群を作成）。未適用の間、アプリは「準備中」と表示して安全に動作します。
